...
![Jot 的 Today 页面：未完成行动、进度细线、收起的已完成区与 AI 对话侧栏](docs/images/chat.png)
...
- 📅 **今天与长期计划分开。** Today 把未完成行动放在前面，已完成的收起来，随手记录单独放着；Todo 管长期事项。在日、周、月日历中查看安排与记录。
- 📓 **不用刻意维护的日志。** 完成行动后自动留下记录和简单统计，也可补写完成说明。普通随手记录独立保存，不会算作完成事项。
- ✨ **AI 帮你整理。** 多套 OpenAI 兼容或 Anthropic 模型配置可以分别保存、测试连接、一键切换；在流式对话里管理记录。新建 Today 行动时，Jot 可以建议关联已有 Todo，由你确认后才会关联；也能选择本地 Codex、Claude 等 Agent 记录交给 AI 总结。
