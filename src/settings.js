...
settingsView=function(){
  ...
  memory.innerHTML=`<h3>Jot 记忆卡</h3><p>只保存对日志助手有用的简短背景；随时可改、可清空。记忆不能代替当前任务记录。</p><label>稳定偏好<textarea name="memoryStable" maxlength="1500" rows="5" placeholder="例如：习惯用中文、喜欢简洁回复。">${esc(state.settings.chat.memoryStable||'')}</textarea></label><p class="hint">由你确认的长期偏好。不会自动从聊天中写入。</p><label>近期摘要<textarea name="memoryRecent" maxlength="1000" rows="6" placeholder="最近关注的事情与已经确认的决定。">${esc(state.settings.chat.memoryRecent||'')}</textarea></label><div class="settings-actions"><button type="button" id="generate-memory">从最近对话生成摘要</button><button type="button" id="undo-memory" hidden>撤销上次记忆修改</button></div><p class="hint">仅在点击时将最近最多 20 条对话发送给当前对话模型；生成后立即保存，可继续修改。不包含本地 Agent 来源记录。</p>`;
  form.append(memory);
  ...
  return layout.outerHTML.replaceAll('选择后点击“保存所有设置”生效','选择后立即生效').replaceAll('保存后从下一条消息生效','修改后从下一条消息生效').replace('Jot 0.6.7','Jot 0.6.10');
};
