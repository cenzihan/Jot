...
function jotTaskRow(t,kind='today'){
  ...
  return `<article class="jot-task ${t.status==='done'?'jot-done':''}"><button class="jot-check" data-complete="${esc(t.id)}" data-kind="${kind}" aria-label="${t.status==='done'?'重新打开':'完成'} ${esc(t.title)}">${t.status==='done'?'✓':''}</button><div class="jot-task-body"><strong>${esc(t.title)}</strong><small>${esc(subtitle)}${t.completionNote&&t.status==='done'?' · 已填写完成说明':''}</small></div><button class="jot-task-edit" data-edit="${esc(t.id)}" data-kind="${kind}" title="编辑与完成说明">···</button></article>`;
}
todayView=function(){
  const key=day(Date.now()), tasks=jotTasksFor(key);
  return `<section class="jot-section"><div class="jot-section-head"><h3>今日行动</h3><span>${tasks.filter(t=>t.status==='done').length} / ${tasks.length}</span></div><div class="jot-task-list">${tasks.length?tasks.map(t=>jotTaskRow(t)).join(''):'<p class="jot-soft-empty">今天还没有行动。</p>'}</div><button class="jot-text-action" data-add="today">＋ 添加今日行动</button></section><section class="jot-section"><div class="jot-section-head"><h3>随手记录</h3><span>不计入完成</span></div>${quickRecord(key)}${recordsView(key)}</section>`;
};

let jotCalendarMode=localStorage.getItem('jotCalendarMode')||'month';
function jotCalendarEntries(key){
  return [...jotTasksFor(key).map(t=>({title:t.title,kind:'today',id:t.id,done:t.status==='done'})),...(state.notes||[]).filter(n=>day(n.recordedAt)===key).map(n=>({title:n.title,kind:'note',id:n.id,done:n.status==='done'})),...state.todos.filter(t=>t.dueAt&&day(t.dueAt)===key&&!t.legacyDdlId).map(t=>({title:'截止 · '+t.title,kind:'deadline',id:t.id,done:t.status==='done',overdue:t.status==='open'&&new Date(t.dueAt).getTime()<Date.now()}))];
}
...
logView=function(){
  ...
  let lastDay='';
  return `<div class="jot-journal-head"><strong>${state.stats.total}</strong><span>累计完成 · 本周 ${state.stats.week} · 今天 ${state.stats.today}</span></div><div class="jot-timeline">${items.map(e=>{const key=day(e.at),heading=key!==lastDay?`<div class="jot-timeline-date">${key===day(Date.now())?'今天':key}</div>`:'';lastDay=key;return `${heading}<article class="jot-timeline-entry ${e.done?'is-done':''}"><span class="jot-timeline-dot"></span><div><strong>${esc(e.title)}</strong><small>${e.done?'已完成':'普通记录'} · ${fmt(e.at)}${e.source==='today'?' · Today':e.source==='todo'?' · Todo':''}</small>${e.notes?`<p>${esc(e.notes)}</p>`:''}</div><div class="row-actions">${e.kind==='note'?`<button data-edit="${esc(e.id)}" data-kind="note" aria-label="编辑记录">✎</button>`:e.entityId?`<button data-edit="${esc(e.entityId)}" data-kind="${esc(e.source)}" aria-label="查看原事项">↗</button>`:`<button data-edit="${esc(e.id)}" data-kind="log" aria-label="编辑完成记录">✎</button>`}</div></article>`;}).join('')||empty('还没有记录','完成行动或随手写下一点什么。','log')}</div>${historyView()}`;
};
